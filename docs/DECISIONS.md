...
- Infra: the 16MB layout is a single 3MB factory app with no OTA slots or `otadata`. The portal lives in a 0xBE0000-byte LittleFS volume at `storage` (offset 0x310000) using the `data/littlefs` partition subtype, which this project’s ESP-IDF/LittleFS component recognizes directly. The final 64KB coredump starts at 0xEF0000 and 1MB remains unallocated for future features. OTA is deliberately deferred and not implemented.
